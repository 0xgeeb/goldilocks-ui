...
export const GovProvider = (props: PropsWithChildren<{}>) => {
  ...
  const [proposeTabState, setProposeTabState] = useState(
    INITIAL_STATE.proposeTab,
  );
  const [newProposalInfoState, setNewProposalInfoState] = useState(
    INITIAL_STATE.newProposalInfo,
  );
  const [govWalletInfoState, setGovWalletInfoState] = useState(
    INITIAL_STATE.govWalletInfo,
  );
  const [govlocksDisplayStringState, setGovlocksDisplayStringState] = useState(
    INITIAL_STATE.govlocksDisplayString,
  );
  const [govlocksActiveToggleState, setGovlocksActiveToggleState] = useState(
    INITIAL_STATE.govlocksActiveToggle,
  );
  ...
  const [govlocksAllowanceButtonsState, setGovlocksAllowanceButtonsState] =
    useState<boolean>(INITIAL_STATE.govlocksAllowanceButtons);
  ...
  const [proposalInfoLoadingState, setProposalInfoLoadingState] =
    useState<boolean>(INITIAL_STATE.proposalInfoLoading);
  ...
  const toggleProposalHover = (proposalNum: number, toggle: boolean) => {
    const updatedProposals = proposalsState.map((proposal: any) =>
      proposal.proposalId === proposalNum
        ? { ...proposal, hover: toggle }
        : proposal,
    );
  ...
  const handleGovlocksBalance = () => {
    if (govlocksActiveToggleState === "WRAP") {
      return govWalletInfoState.locks > 0
        ? govWalletInfoState.locks.toLocaleString("en-US", {
            maximumFractionDigits: 4,
          })
        : "0.00";
    }
    if (govlocksActiveToggleState === "UNWRAP") {
      return govWalletInfoState.govlocks > 0
        ? govWalletInfoState.govlocks.toLocaleString("en-US", {
            maximumFractionDigits: 4,
          })
        : "0.00";
    }
    if (govlocksActiveToggleState === "DELEGATE") {
      return govWalletInfoState.votes > 0
        ? govWalletInfoState.votes.toLocaleString("en-US", {
            maximumFractionDigits: 4,
          })
        : "0.00";
    }
  ...
  const refreshProposals = async (proposalData: any) => {
    const processedProposals = proposalData.proposals.items.map(
      (proposal: any) => {
        const colonIndex = proposal.description.indexOf(":");
        const title =
          colonIndex !== -1
            ? proposal.description.slice(0, colonIndex).trim()
            : "";
        const description =
          colonIndex !== -1
            ? proposal.description.slice(colonIndex + 1).trim()
  ...
  const refreshProposalInfo = async (proposalId: number) => {
    ...
    const threshold = parseFloat(
      formatEther(thresholdResult as unknown as bigint),
    );
    const proposerVotes = parseFloat(
      formatEther(proposerVotesResult as unknown as bigint),
    );

    const cancelResult =
      getProposalStateString(parseFloat(stateResult as unknown as string)) !==
        "EXECUTED" &&
      (proposalResult[0] === address || proposerVotes < threshold);
    const queueResult =
      getProposalStateString(parseFloat(stateResult as unknown as string)) ===
      "SUCCEEDED";
    const executeResult =
    ...
    const response = {
      state: getProposalStateString(
        parseFloat(stateResult as unknown as string),
      ),
      forVotes: parseFloat(formatEther(proposalResult[5] as unknown as bigint)),
      againstVotes: parseFloat(
        formatEther(proposalResult[6] as unknown as bigint),
      ),
      abstainVotes: parseFloat(
        formatEther(proposalResult[7] as unknown as bigint),
      ),
  ...
  const refreshGovWalletInfo = async () => {
    if (address) {
      ...
      const response = {
        ...
        locksGovlocksAllowance: parseFloat(
