...
export const ProposalButtons = ({ number }: ButtonsProps) => {
  const { proposalInfo, proposals, proposalInfoLoading, infoLoading } =
    useGov();

  const { chain, isConnected } = useAccount();

  const { sendCancelTx, sendQueueTx, sendExecuteTx } = useGovTx();
  ...
  return (
    <div className="absolute right-[2.5%] top-[2.5%] flex h-[8%] w-[35%] flex-row items-center justify-between font-amaticbold text-[3.25vw] font-medium lg:right-[5%] lg:top-[8%] lg:h-[10%] lg:w-[25%] lg:text-[1.5vw]">
      {infoLoading || proposalInfoLoading ? (
        loadingElement()
      ) : (
        <>
          <div
            className={`h-[100%] w-[30%] border-2 border-black ${proposalInfo.cancel && proposal ? "cursor-pointer bg-[#D9C6BA] hover:scale-110" : "bg-gray-300 opacity-30"} flex items-center justify-center`}
            id="cancel"
          ...
          <div
            className={`h-[100%] w-[30%] border-2 border-black ${proposalInfo.queue ? "cursor-pointer bg-[#D9C6BA] hover:scale-110" : "bg-gray-300 opacity-30"} flex items-center justify-center`}
            id="queue"
          ...
          <div
            className={`h-[100%] w-[30%] border-2 border-black ${proposalInfo.execute ? "cursor-pointer bg-[#D9C6BA] hover:scale-110" : "bg-gray-300 opacity-30"} flex items-center justify-center`}
            id="execute"
