...
export const GovLocksBox = () => {
  ...
  const wrapTxFlow = async (button: HTMLElement | null) => {
    ...
    if(wrap > govWalletInfo.locks) {
      ...
      const sufficientAllowance: boolean | void = await checkAllowance(wrap, address as string)
  ...
  const renderButton = () => {
    if(govlocksActiveToggle === 'WRAP') {
      if(isConnected && wrap > govWalletInfo.locksGovlocksAllowance && govWalletInfo.locks >= wrap) {
  ...
  return (
    <div className="absolute top-[12.5%] lg:top-[7.5%] right-[10%] lg:right-[5%] w-[80%] lg:w-[60%] h-[75%] border-2 border-black bg-[#EEDCD2]">
      ...
      <div className="absolute inset-6 border-2 border-black bg-[#D9C6BA]">
        <div className="w-[100%] h-[100%] flex flex-col py-[1.5%]">
          <div className="w-[90%] lg:w-[75%] flex flex-row justify-between items-center mx-auto font-amaticbold font-medium text-[4vw] lg:text-[1.75vw]">
            <h1 className={`cursor-pointer hover:scale-125 ${govlocksActiveToggle === 'WRAP' ? "underline" : ""}`} onClick={() => changeGovlocksActiveToggle('WRAP')}>WRAP</h1>
            <h1 className={`cursor-pointer hover:scale-125 ${govlocksActiveToggle === 'UNWRAP' ? "underline" : ""}`} onClick={() => changeGovlocksActiveToggle('UNWRAP')}>UNWRAP</h1>
            <h1 className={`cursor-pointer hover:scale-125 ${govlocksActiveToggle === 'DELEGATE' ? "underline" : ""}`} onClick={() => changeGovlocksActiveToggle('DELEGATE')}>DELEGATE</h1>
          </div>
          <div className="h-[25%] lg:h-[30%] w-[70%] lg:w-[60%] mx-auto border-2 border-black bg-white my-[10%] lg:my-[5%]">
            <div className="relative h-[100%] w-[100%]">
              {
                govlocksActiveToggle === "DELEGATE" ?
                <input
                  className="absolute top-[15%] pl-[5%] w-[90%] focus:outline-none border-none bg-transparent font-bold font-baloo text-[4vw] lg:text-[2vw]"
                  type="string"
                  placeholder=""
              ...
              <span className={`absolute bottom-0 right-[3%] font-baloo font-bold text-[2.25vw] lg:text-[0.9vw] text-[#7F7F7F] ${govlocksActiveToggle === "DELEGATE" ? "" : "cursor-pointer hover:scale-125"}`} onClick={() => handleGovlocksBalanceClick()}>
                {govlocksActiveToggle === "WRAP" ? "locks" : govlocksActiveToggle === "UNWRAP" ? "govlocks" : "votes"}: {walletInfoLoading ? loadingElement() : handleGovlocksBalance()}
              </span>
            </div>
          </div>
          {
            govlocksAllowanceButtons &&
            <div className="flex flex-row items-center justify-between mx-auto w-[60%] lg:w-[50%] h-[12.5%]">
              <button
                className="bg-[#E7B941] h-[100%] w-[46%] lg:w-[48%] border-2 border-black font-amaticbold text-[3vw] lg:text-[1.5vw] hover:bg-[#B35227] hover:text-[#E7B941] hover:scale-110"
          ...
          }
          {
            ...
            <ConnectButton.Custom>
              {({
                ...
                return (
                  <button
                    className={`mx-auto h-[12.5%] w-[36%] ${buttonLoadingColor ? "bg-[#B35227] text-[#E7B941]" : "bg-[#E7B941] text-black"} hover:bg-[#B35227] hover:text-[#E7B941] font-amaticbold text-[4vw] lg:text-[1.9vw] border-2 border-black hover:scale-110`}
                    id="govlocks-button"
                    onClick={() => {
                      const button = document.getElementById('govlocks-button')
