...
const Notification = ({ hash, title, direction, amount, price, page }: NotificationProps) => {
  ...
  return (
    <div className="flex flex-col py-8 items-center text-center font-acme">
      <span 
      ...
      <h1 className="text-[1.5rem] mb-4">{
        page === 'amm' ? 
          `you ${direction} ${amount.toLocaleString('en-US', { maximumFractionDigits: 4 })} $LOCKS for ${price.toLocaleString('en-US', { maximumFractionDigits: 4 })} $HONEY` :
        page === 'stake' ? 
        ...
        page === 'goldilend-claim' ?
          `you ${direction} $PRG` :
        page === 'goldilend-liq' ?
          `you ${direction} loan #${price} with ${amount.toLocaleString('en-US', { maximumFractionDigits: 4 })} $BERA` :
        page === 'morehoney' ?
          `you need ${amount.toLocaleString('en-US', { maximumFractionDigits: 4 })} $honey to do dis` :
        ''
      }</h1>
      {page !== 'morehoney' && <a href={`https://artio.beratrail.io/tx/${hash}`} target="_blank">
