...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-black/20 text-white flex flex-col gap-6 rounded-xl border border-amber-900/30 py-6 shadow-sm",
        className
      )}
      {...props}
    />
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div data-slot="card-description" className={cn("text-white/70 text-sm", className)} {...props} />
  );
}
